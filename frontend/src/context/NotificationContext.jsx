  ...
  return (
    <NotificationContext.Provider value={{ showNotification }}>
      ...
      {toasts.length > 0 && createPortal(
        <div className="fixed top-6 left-1/2 transform -translate-x-1/2 z-[9999] px-4 w-full max-w-sm flex flex-col gap-3 pointer-events-none">
          {toasts.map((toast) => {
            const current = config[toast.type] || config.success;
            return (
              <div
                key={toast.id}
                className={`flex items-center gap-3 px-4 py-3 rounded-2xl border shadow-xl backdrop-blur-md transition-all duration-300 pointer-events-auto animate-scale-up ${current.bgColor} ${current.borderColor} ${current.glow}`}
              >
                {current.icon}
                <p className={`text-sm font-semibold leading-relaxed grow ${current.textColor}`}>{toast.message}</p>
                <button
