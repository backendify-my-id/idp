  ...
  return (
    <div className="space-y-8 animate-fade-in text-left">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {/* Metric 1 */}
        <div className="p-5 border border-slate-200/50 dark:border-slate-800/80 bg-white/50 dark:bg-slate-900/40 backdrop-blur rounded-2xl flex items-center justify-between gap-4 shadow-sm hover:scale-[1.01] transition-transform">
          <div className="text-left">
            <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Database Users</span>
            <h4 className="text-2xl font-black text-slate-800 dark:text-white mt-1">{usersCount || 148}</h4>
            <span className="text-[8px] font-extrabold text-emerald-500 uppercase tracking-wide block mt-1">+12% this month</span>
          </div>
          <div className="w-11 h-11 rounded-xl bg-indigo-50 dark:bg-indigo-950/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 shrink-0">
        ...
        <div className="p-5 border border-slate-200/50 dark:border-slate-800/80 bg-white/50 dark:bg-slate-900/40 backdrop-blur rounded-2xl flex items-center justify-between gap-4 shadow-sm hover:scale-[1.01] transition-transform">
          <div className="text-left">
            <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">OIDC Clients</span>
            <h4 className="text-2xl font-black text-slate-800 dark:text-white mt-1">{clientsCount || 6}</h4>
            <span className="text-[8px] font-extrabold text-indigo-500 uppercase tracking-wide block mt-1">PKCE flow enabled</span>
          </div>
          <div className="w-11 h-11 rounded-xl bg-purple-50 dark:bg-purple-950/30 flex items-center justify-center text-purple-600 dark:text-purple-400 shrink-0">
            <svg className="w-5.5 h-5.5" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
        ...
        <div className="p-5 border border-slate-200/50 dark:border-slate-800/80 bg-white/50 dark:bg-slate-900/40 backdrop-blur rounded-2xl flex items-center justify-between gap-4 shadow-sm hover:scale-[1.01] transition-transform">
          <div className="text-left">
            ...
            <h4 className="text-2xl font-black text-slate-800 dark:text-white mt-1">{activeSessions}</h4>
            <span className="text-[8px] font-extrabold text-emerald-500 uppercase tracking-wide block mt-1">WS active sockets</span>
          </div>
          <div className="w-11 h-11 rounded-xl bg-cyan-50 dark:bg-cyan-950/30 flex items-center justify-center text-cyan-600 dark:text-cyan-400 shrink-0">
            <svg className="w-5.5 h-5.5" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
        ...
        <div className="p-5 border border-slate-200/50 dark:border-slate-800/80 bg-white/50 dark:bg-slate-900/40 backdrop-blur rounded-2xl flex items-center justify-between gap-4 shadow-sm hover:scale-[1.01] transition-transform">
          <div className="text-left">
            <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Core Network</span>
            <div className="flex items-center gap-1.5 mt-2">
              <span className={`w-2 h-2 rounded-full ${socketStatus === 'connected' ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'}`} />
              <h4 className="text-xs font-black text-slate-850 dark:text-white uppercase leading-none">
                {socketStatus === 'connected' ? 'Online' : 'Fallback'}
              </h4>
            </div>
            <span className="text-[8px] font-extrabold text-slate-400 uppercase tracking-wide block mt-1.5">Gateway operational</span>
          </div>
          ...
          <div className="h-[210px] overflow-y-auto scrollbar-thin space-y-2 pr-1">
            {wsLogs.length === 0 ? (
              ...
              wsLogs.slice(0, 10).map((log, idx) => (
                <div key={idx} className="p-2.5 bg-slate-100/30 dark:bg-slate-900/10 border border-slate-150/20 dark:border-slate-800/20 rounded-xl flex items-start gap-3 text-xs">
                  <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-1.5 shrink-0" />
                  <p className="font-semibold text-slate-650 dark:text-slate-350 tracking-tight leading-relaxed">{log}</p>
                </div>
              ))
            )}
          </div>
